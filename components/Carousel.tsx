...
const Slide = (props: SlideProps) => {
  ...
  return (
    <div
      key={key}
      class={`${props.class} ${color} h-100 w-full text-center text-black`}
    >
      {text}
...
const Carousel = (props: CarouselProps) => {
  ...
  const DotsNavigation = () => (
    <div class={"slide_nav z-30 w-full absolute bottom-6 flex justify-center gap-8"}>
        ...
        return (
          ...
          >
            ...
            {idx === currentSlide.value
              ? <span class="not-sr-only block w-20 h-2 rounded-lg bg-white "></span>
              : <span class="not-sr-only block w-20 h-2 rounded-lg bg-white bg-opacity-30 border border-white border-opacity-30"></span>}
          </button>
        );
      })}
    </div>
